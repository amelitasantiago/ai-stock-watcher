...
class ModelConfig:
    ...
    enable_eval: bool = True  # FIXED: Toggle evaluation phase
    max_iter: int = 300  # For ARIMA convergence
...
class DataProcessor:
    ...
    def load_sentiment_data(self, ticker: str) -> pd.DataFrame:
        """Load sentiment data for a specific ticker with backfill for historical coverage.
            Args:
                ticker (str): Stock ticker symbol (e.g., 'AAPL').            
            Returns:
                pd.DataFrame: Indexed by date, with columns ['avg_sentiment', 'avg_magnitude', 'article_count'].
                            Handles sparsity via neutral baseline + EMA decay for momentum modeling.            
            Raises:
                sqlite3.Error: If DB query fails.        
        IRS Alignment: Enables multi-modal reasoning by ensuring sentiment persistence across regimes.
        """
    ...
    def merge_with_sentiment(self, stock_df: pd.DataFrame, sentiment_df: pd.DataFrame) -> pd.DataFrame:
        ...
        merged_df = stock_df.join(sentiment_df, how='left')
        ...
        for col in ["avg_sentiment", "avg_magnitude", "article_count"]:
            merged_df[f"{col}_t1"] = merged_df[col].shift(1)

        # rolling means (backward looking)
        merged_df["sent_3d_mean"] = merged_df["avg_sentiment"].rolling(3, min_periods=1).mean().shift(1)
        merged_df["sent_7d_mean"] = merged_df["avg_sentiment"].rolling(7, min_periods=1).mean().shift(1)

        # Drop same-day raw columns to avoid look-ahead leakage
        merged_df = merged_df.drop(columns=["avg_sentiment", "avg_magnitude", "article_count"], errors="ignore")

        # Fill only initial small holes
        merged_df = merged_df.fillna(0)

        usable = int((merged_df["sent_7d_mean"] != 0).sum())
        logging.info(f"Merged sentiment (backward-only): {usable} days with usable sentiment features")
        return merged_df
    ...
    def prepare_features(self, ticker: str, config: ModelConfig) -> pd.DataFrame:
        ...
        # Simple autoregressive signal features
        stock_df["ret_1d"] = stock_df["close"].pct_change()
        for L in [1,2,3,5,10]:
            stock_df[f"ret_lag_{L}"] = stock_df["ret_1d"].shift(L)
        for W in [5,10,20]:
            stock_df[f"ret_mean_{W}"] = stock_df["ret_1d"].rolling(W).mean()
            stock_df[f"ret_std_{W}"]  = stock_df["ret_1d"].rolling(W).std()
        ...
        # after creating targets, drop tail NaNs caused by the shift: Do NOT use this version unless refactored DataProcessor to own the config:
        #target_cols = [f'target_{h}d' for h in range(1, self.config.prediction_horizon + 1)]
        #stock_df = stock_df.dropna(subset=target_cols + ['close'])
...
class HybridStockPredictor:
    ...
    def train(self, ticker: str, data_processor: DataProcessor) -> bool:
        ...
        try:
            ...
            if STATSMODELS_AVAILABLE:
                try:
                    price_series = df['close'].dropna().copy()
                    if len(price_series) > 0:
                        # Ensure DatetimeIndex with business-day frequency
                        if not isinstance(price_series.index, pd.DatetimeIndex):
                            price_series.index = pd.to_datetime(price_series.index)
                        price_series = price_series.sort_index().asfreq("B").ffill()
                        ...
                        # Fit ARIMA on price level (keeps units consistent with Ridge/LSTM for ensemble)
    ...
    def train_lstm_transformer(self, X: np.ndarray, y: np.ndarray) -> Optional[Tuple[nn.Module, optim.Optimizer, Dict]]:
        """Train LSTM-Transformer with chronological Train/Val/Test split and proper scaling."""
        if not TORCH_AVAILABLE:
        ...
        try:
            ...
            if n_train == 0 or n_val == 0 or n_test == 0:
                logging.warning(f"Insufficient samples for split: n={n}")
                return None
            ...
            # Tensors + loaders (no shuffle for time series)
            ...
            bs = min(self.config.batch_size, max(1, len(train_ds)))
            ...
            # Evaluate on TEST (inverse-transform to original units)
            def inv_y(arr):
            ...
            logging.info(f"[LSTM][TEST] MAE@1={mae:.4f} RMSE@1={rmse:.4f} R2@1={r2:.3f}")

            # persist scalers for prediction-time inverse transform
            self.lstm_scalers = {"X": scaler_X, "y": scaler_y}
            return model, optimizer, {"loss": last_train, "val_loss": best_val, "test_mae_h1": mae, "test_rmse_h1": rmse, "test_r2_h1": r2}
    ...
    def train_bayesian_ridge(self, X: np.ndarray, y_multi: np.ndarray) -> Tuple[Dict[int, BayesianRidge], List[float]]:
        ...
        # Cross-validated evaluation with scaler fit on TRAIN in each fold
        ...
        for h in range(1, self.config.prediction_horizon + 1):
            try:
                ...
                y_h = y_multi[:, h-1]
                if X.shape[0] < max(10, int(X.shape[1]*1.5)):
                ...
                for train_idx, test_idx in tscv.split(X):
                    ...
                    model_h.fit(X_tr_s, y_tr)

                    y_hat = model_h.predict(X_te_s)
                    fold_r2.append(r2_score(y_te, y_hat))
                    fold_mae.append(mean_absolute_error(y_te, y_hat))

                # store CV summaries
                r2_scores.append(float(np.nanmean(fold_r2)))
                val_mae_all.append(float(np.nanmean(fold_mae)))

                # retrain on FULL data for deployment, with scaler fitted on FULL X
                scaler_full = StandardScaler().fit(X)
                model_full  = BayesianRidge(
                    n_iter=self.config.max_iter,
                    alpha_1=self.config.alpha_1, alpha_2=self.config.alpha_2,
                    lambda_1=self.config.lambda_1, lambda_2=self.config.lambda_2,
                    fit_intercept=True, copy_X=True, compute_score=False
                ).fit(scaler_full.transform(X), y_h)

                models_tmp[h] = (model_full, scaler_full)

                logging.info(f"Ridge h{h}: CV R²={np.nanmean(fold_r2):.3f}, CV MAE={np.nanmean(fold_mae):.4f}")

            except Exception as e:
                logging.error(f"Bayesian Ridge training failed for horizon {h}: {e}")
                continue

        # Persist models and the feature scaler(s) used for prediction
        self.bayesian_models = {h: m for h, (m, s) in models_tmp.items()}
        self.scalers['ridge_features'] = {h: s for h, (m, s) in models_tmp.items()}
        avg_r2 = np.nanmean(r2_scores) if r2_scores else float('nan')
        logging.info(f"Bayesian Ridge: {len(self.bayesian_models)} horizons trained, avg CV R²: {avg_r2:.3f}")
    ...
    def predict(self, recent_data: np.ndarray, price_series: pd.Series = None) -> Dict[str, np.ndarray]:
        ...
        if self.lstm_model is not None and TORCH_AVAILABLE:
            try:
                ...
                with torch.no_grad():
                    ...
                    # This fixes the units mismatch so the ensemble combines price-level predictions across ARIMA/Ridge/LSTM.
                    lstm_pred = self.lstm_model(input_tensor).detach().numpy()[0]
                    # Inverse-transform to original units if scaler is available
                    try:
                        sy = self.lstm_scalers.get("y")
                        if sy is not None:
                            lstm_pred = (lstm_pred * sy.scale_) + sy.mean_
                    except Exception:
                        pass
        ...
        if self.bayesian_models:
            try:
                ...
                # Build a single (1, n_features) row from the latest timestep
                if recent_data is None:
                    recent_vec = np.zeros((1, len(self.feature_columns)), dtype=float)
                elif recent_data.ndim == 3:
                    # (n_seq, seq_len, n_feat) -> take last time step of last sequence
                    recent_vec = recent_data[-1, -1, :].reshape(1, -1)
                elif recent_data.ndim == 2:
                    # (n_samples, n_feat) -> take last sample
                    recent_vec = recent_data[-1, :].reshape(1, -1)
                else:
                    recent_vec = np.array(recent_data).reshape(1, -1)

                # Apply the same scaling used during training
                scaler = self.scalers.get('ridge_features')
                recent_scaled = scaler.transform(recent_vec) if scaler is not None else recent_vec
                if scaler is None:
                    logging.warning("No Ridge feature scaler found, using unscaled data")
                ...
                for h in range(1, self.config.prediction_horizon + 1):
                    if h in self.bayesian_models:
                        br_predictions.append(self.bayesian_models[h].predict(recent_scaled)[0])
                    else:
                        br_predictions.append(0.0)
                        predictions['ridge'] = np.array(br_predictions)
